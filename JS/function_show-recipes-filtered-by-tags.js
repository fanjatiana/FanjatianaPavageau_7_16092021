...
export const showRecipesFiltered = (allTags, dataFiltered) => {
  ...
  allTags.forEach((tag) => {
    const resultFilterByIngredients = dataFiltered.filter((recipe) => recipe.ingredients.map((list) => list.ingredient
      .toLowerCase()
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, ''))
      .includes(
    ...
    const resultFilterByAppliances = dataFiltered.filter((recipe) => recipe.appliance.toLowerCase()
      .normalize('NFD')
      .replace(/[\u0300-\u036f]/g, '')
      .includes(tag.toLowerCase()
        .normalize('NFD')
        .replace(/[\u0300-\u036f]/g, '')));
    const resultFilterByTools = dataFiltered.filter((recipe) => recipe.ustensils.map((list) => list.toLowerCase()
      .normalize('NFD')
